...
- **Beads board** — a read-only board view (top-bar toggle, plus a mobile tab) showing the repo's Beads work in ready, in-progress, blocked, and recently-closed columns, each bounded with a count. Within a column, rows are grouped by assignee (compact label; an `Unassigned` bucket sinks to the bottom) when any row carries one, else listed flat. Reuses Beads + Dolt as the work ledger rather than a separate store: the daemon shells `bd ready` / `bd list --json` for its own repo behind `GET /beads/board` (short-cached to coalesce refreshes). The dashboard never writes to Beads. When the `bd` CLI is missing or the repo has no Beads workspace, the panel shows a quiet unavailable state.
