...
Restart now resumes the backend's prior conversation when possible. If the peer has a captured runtime session id (claude-code and codex capture it from the hook payload) and a matching session file exists on disk, restart relaunches with the backend's native resume command (`claude --resume <id>` / `codex resume <id>`) in the original working directory and reports `resume_mode=resumed`. Otherwise it restarts fresh (`resume_mode=fresh_runtime_context`) and reports a `resume_warning` explaining why (no captured id, backend doesn't support resume, or the captured id has no local session file — stale/expired). The session id is pre-validated on disk **before** the pane is killed, because both Claude and Codex exit non-zero on an unknown id rather than starting fresh; resuming a stale id after killing the pane would leave the peer dead. Resume does not persist the originally selected spawn profile. For peers Repowire cannot prove it spawned (no pane ownership), restart still refuses to kill the pane but returns the exact `resume_command` in the 409 detail so you can relaunch with context manually.
