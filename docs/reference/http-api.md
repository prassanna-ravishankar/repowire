...
- `/peers` for peer registration, listing, lookup, and lifecycle operations. Peer records include optional `model`, the last observed runtime model reported by the backend, and flattened provenance: `source` (`hook`, `codex-app-server`, `unknown`), `parent_runtime_id`, `parent_peer_id` (resolved at read time, `null` when the parent is not registered), `ephemeral`, `addressable`, and `addressable_reason`. `GET /peers` is a complete inventory; `?source=` and `?addressable=true|false` narrow it. `POST /peers` accepts an optional `provenance` object as a fresh classification (omitting it preserves what the daemon knows, and only an unknown `source` is inferred from `metadata.transport` / `hook_version`). `POST /peers/{id}/provenance` replaces a live peer's provenance and emits a `peer_updated` event; the codex bridge uses it to demote a thread whose direct input the App Server denied. Includes `GET /peers/{id}/doctor` (read-only diagnostic report with contradiction detection) and `POST /peers/{id}/rehook` (non-destructive inbound ws-hook recovery, same-host, dry-run by default).
