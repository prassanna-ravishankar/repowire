...
- `GET /beads/board` returns a read-only, bounded snapshot of the repo's Beads work (ready / in-progress / blocked / recently-closed) for the dashboard board view. Short-cached to coalesce refreshes; only `bd ready` / `bd list` are shelled (never a write or sync verb); reports `available:false` when Beads is unavailable.
