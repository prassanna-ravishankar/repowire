...
- `/peers` for peer registration, listing, lookup, and lifecycle operations. Peer records include optional `model`, the last observed runtime model reported by the backend, and flattened provenance: `source` (`hook`, `codex-app-server`, `unknown`), `initiator` (`user`, `agent`, `system`, or absent), `parent_runtime_id`, `parent_peer_id` (resolved at read time, `null` when the parent is not registered), `ephemeral`, `addressable`, and `addressable_reason`. `GET /peers` is a complete inventory; `?source=`, `?addressable=true|false`, and `?listed=true|false` (the routing view's rule: addressable, not `system`, parent live if any) narrow it. `POST /peers` accepts an optional `provenance` object as a fresh classification (omitting it preserves what the daemon knows, and only an unknown `source` is inferred from `metadata.transport` / `hook_version`). `POST /peers/{id}/provenance` replaces a live peer's provenance and emits a `peer_updated` event; the codex bridge uses it to demote a thread whose direct input the App Server denied. A peer with `addressable=false` is inbound-only: `POST /ask` and `POST /notify` to it return **409** `{"error"|"reason": "peer_not_addressable", "peer_id", "parent_peer_id" (null when the parent is not registered), "hint"}` before anything is registered or queued, broadcasts skip it, and scheduled asks fail the same way. It can still ack, reply, notify, and broadcast. When a live peer loses addressability the daemon closes its open inbound asks with reason `peer_not_addressable`, drops its queued deliveries, notifies each asker, and records one `peer_not_addressable` event with the counts. Includes `GET /peers/{id}/doctor` (read-only diagnostic report with contradiction detection) and `POST /peers/{id}/rehook` (non-destructive inbound ws-hook recovery, same-host, dry-run by default).
...
- `/ask`, `/ack`, `/decline`, and `/asks/pending` for ask lifecycle. `GET /asks/history?pane_id=|peer_id=&since=RFC3339` lists the closed ask threads a peer took part in (as asker or recipient) after `since`, read from the durable trace ledger rather than the in-memory tracker, plus the peer's `repowire_session_id`. Backs the git-trailer hook and `repowire why`. `POST /decline` closes an unhandled ask with `close_reason="declined"` and returns the reason to its asker.
