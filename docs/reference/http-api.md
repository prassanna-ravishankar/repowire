...
- `/ask`, `/ack`, `/decline`, and `/asks/pending` for ask lifecycle. `POST /decline` closes an unhandled ask with `close_reason="declined"` and returns the reason to its asker.
