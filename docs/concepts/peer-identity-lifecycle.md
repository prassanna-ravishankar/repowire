...
The in-memory `Peer` is the live routing record. The durable `SessionMapping` preserves fields that should survive daemon restart, including display name, circle, backend, path, role, description, and the last known agent pid when available. Fresh tmux, tmux-window, or spawn placement overrides a stored circle when a runtime moves; the stored circle is only the fallback when no current placement exists. Those mappings live in `~/.repowire/state.db`; legacy `sessions.json` is imported once and then left untouched for downgrade/export compatibility.
